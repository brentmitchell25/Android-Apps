...
public class Map {
	...
	public Map(Resources res, int id, 
		int init_res_x_top, int init_res_y_top, 
		int init_res_x_bot, int init_res_y_bot,
		int view_width, int view_height) {
		
		this.res = res;
		res_id = id;
		
		initRegion(init_res_x_top, init_res_y_top, init_res_x_bot, init_res_y_bot);
		
		subset_width = view_width;
		subset_height = view_height;
		
		initializeReferenceOptions();
		reloadReference(init_res_x_bot - init_res_x_top, init_res_y_bot - init_res_y_top);
		//reference_map = BitmapFactory.decodeResource(res, res_id, reference_options);
	...
	// Constructs a new map based on the resource and id. Initially centers the view.
	public Map(Resources res, int id, int view_width, int view_height) {
		...
		initRegion((reference_default_width - subset_width)/2, (reference_default_height - subset_height)/2,
				(reference_default_width + subset_width)/2, (reference_default_height + subset_height)/2);
	...
	private synchronized void initRegion(int x_top, int y_top, int x_bot, int y_bot) {
		rx1 = x_top;
		ry1 = y_top;
		rx2 = x_bot;
		ry2 = y_bot;
	...
	public synchronized void setRegion(float x_top, float y_top, float x_bot, float y_bot) {
	...
	public synchronized void moveRegion(float dx_top, float dy_top, float dx_bot, float dy_bot) {
		...
		if (new_width > reference_default_width || new_height > reference_default_height) return;
	...
	/*public synchronized void rotate90() {
		...
		/*x1 = (int)(3*(temp[0]-cx) + 2*(cy-temp[1]));
		y1 = (int)(3*(temp[1]-cy) + 2*(cx-temp[0]));
		
		x2 = (int)(3*(temp[2]-cx) + 2*(cy-temp[3]));
		y2 = (int)(3*(temp[3]-cy) + 2*(cx-temp[2]));
		...
		if (isLandscape()) {
			Log.v("center","rotation horiz -> vert");
			x1 = (int)(cy + cx) - temp[3];
			y1 = (int)(cy - cx) + temp[0];
			x2 = (int)(cy + cx) - temp[1];
			y2 = (int)(cy - cx) + temp[2];
		...
		int temp2 = subset_width;
		subset_width = subset_height;
		subset_height = temp2;
		
		w = x2 - x1;
		h = y2 - y1;
		Log.v("center","rotation cent-x: " + cx + " | " + (w/2f + x1));
		Log.v("center","rotation cent-y: " + cy + " | " + (h/2f + y1));

		Log.v("rotate", "rotate top (" + temp[0] + ", " + temp[1] + ") -> (" + x1 + ", " + y1 + ")");
		Log.v("rotate", "rotate bot (" + temp[2] + ", " + temp[3] + ") -> (" + x2 + ", " + y2 + ")");
	...
	public synchronized void transformImage() {
		...
		// Multipliers to rescale the points to the current scale of reference_map after we reload it
		final float multiplier1 = reference_options.outWidth/((float)reference_default_width);
		final float multiplier2 = reference_options.outHeight/((float)reference_default_height);
		...
		// now we get the multipliers to map the raw x and y coordinates to fit the screen
		final float scale_w = ((float)tw)/subset_width;
		final float scale_h = ((float)th)/subset_height;
				
		// Scale the image
		// Read from reference_map and write to map one row of pixels at a time to save
	...
	private void initializeReferenceOptions() {
		...
		// Set the outWidth and outHeight to the width and height of the original image
		...
		// Set impossible sample size to force reloadReference to load the image on
		// its first pass
		reference_options.inSampleSize = -1;
	}
	
	private synchronized boolean reloadReference(int area_width, int area_height) {
		// TODO: Make the n added to the calculated sample size dependent on screen pixel
		// 		 density.
		int calculated_sample_size = calculateSampleSize(area_width, area_height)+1;
		
		if (calculated_sample_size != reference_options.inSampleSize && calculated_sample_size >= 1) {
