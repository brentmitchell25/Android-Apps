...
public class MapTouchHandler {
	...
	public static boolean handleTouch(MapSurface s, Map m, MotionEvent e) {
		...
		switch(action) {
			...
			case(MotionEvent.ACTION_DOWN): {
				...
				//active_list.add(new TouchCoord((int)MotionEventCompat.getX(e, index), (int)MotionEventCompat.getY(e, index)));
				updateCenter(e, true);
			...
			case(MotionEvent.ACTION_UP): {
				if (touches == 1) {
					if (!(checkState(STATE_BIT_MULTI_PAST) || checkState(STATE_BIT_MOVED))) {
						m.getTouchManager().dispatchClick(s, m.transformScreenX(e.getX()), m.transformScreenY(e.getY()));
			...
			case(MotionEvent.ACTION_MOVE): {
				//active_list.get(index).update((int) MotionEventCompat.getX(e, index), (int) MotionEventCompat.getY(e, index));
				...
				if (touch_center.getDX() != 0 || touch_center.getDY() != 0 || 
						distance_measure.getDX() != 0 || distance_measure.getDY() != 0) {
					...
					m.moveRegion(distance_measure.getDX()*dx - touch_center.getDX(), distance_measure.getDX()*dy - touch_center.getDY(),
							-(distance_measure.getDX()*dx + touch_center.getDX()), -(distance_measure.getDX()*dy + touch_center.getDY()));
					
					if (MapSurface.getInstance().getState() == MapSurface.RenderSurfaceThread.THREAD_IDLE) {
						MapSurface.getInstance().requestRedraw();
					}
				}
				//m.moveRegion(distance_measure.getDX(), distance_measure.getDX(), -distance_measure.getDX(), -distance_measure.getDX());
				return true;
			}
			...
			default: {
				Log.w("Touch Event", "Touch event not handled. Action Mask: " + action );
			}
		}
	...
	private static void setStateBit(byte bit) {
		...
		state |= bit;
		Log.v("State Change", "state added: " + decompose(bit) + " | " + decompose(temp_state) + "->" + decompose(state));
	}
	
	private static void unSetStateBit(byte bit) {
		byte temp_state = state;
		state &= ~bit;
		Log.v("State Change", "state removed: " + decompose(bit) + " | " + decompose(temp_state) + "->" + decompose(state));
	}
	...
	private static void updateCenter(MotionEvent e, boolean double_update) {
		...
		if (touches > 0) {
			...
			//Log.d("thing", "check: " + touch_center.update(sumx/touches, sumy/touches));
			if (double_update) {
